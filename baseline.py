...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 10:34:38 2024
...
# Define the objective function
    obj_expr = pulp.lpSum(
        ...
        t * Ps * (1 - x[i]) * x_prev[i] +
        t * Pa * (1 - x_prev[i]) * x[i] for i in range(n)
    )
...
# define the constraint for the CU
    constraint_expr = pulp.lpSum(
        ...
        t * c_vec[i] * (1 - x_prev[i]) * x[i] for i in range(n)
    ) >= n*c*t*alpha
...
#solve P
    ...
    obj = np.sum([t * Pa * x[i].value() * x_prev[i] +
    t * Ps * (1 - x[i].value()) * (1 - x_prev[i]) +
    toff * Poff * (1 - x[i].value()) * x_prev[i] +
    (t-toff) * Ps * (1 - x[i].value()) * x_prev[i] +
    ton * Pon * (1 - x_prev[i]) * x[i].value() +
    (t-ton) * Pa * (1 - x_prev[i]) * x[i].value() for i in range(n)])
...
for i in range(len(ALPHA)):
    ...
    basehistory.append(y)
    baseActive.append(np.sum(y))
    
    baseObj.append(run[1])
    baseMarge.append(run[2] - n*c*t*ALPHA[i])