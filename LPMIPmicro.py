...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 18 11:50:38 2024
...
def LP(L,n1,n2):
    ...
    prob += pulp.lpSum(f[i] for i in range(n1)) + pulp.lpSum(T-Ton-y[i] for i in range(n2)) >= L/C
    
    
            
    glpk_solver = pulp.GUROBI()
    ...
    return new_n1, new_n2, adecision, sdecision, active_DUs, energy, cons, values
...
LPobjmicro = []
LPconsmicro=[]
LPsleepToactivemicro=[]
LPactiveTosleepmicro=[]
LPValuesmicro=[]
...
for i in range(len(ALPHA)):
    ...
    #LPobj.append(linear[5])
    
    waste = linear[6] - n*C*T*ALPHA[i]
    savings = math.floor(waste/C)*(Pa-Pmicro)
    
    LPobjmicro.append(linear[5]-savings)
    
    LPconsmicro.append(waste - C*math.floor(waste/C))
    
    LPserversmicro.append(sum(linear[4]) - math.floor(waste/C)/T)
    
    LPsleepToactivemicro.append(linear[3])
    LPactiveTosleepmicro.append(linear[2])
    LPValuesmicro.append(linear[7])
